...
const pwnya = 'https://raw.githubusercontent.com/rafaelputrasitinjak23/rafael-putra/main/database.json';
const pwinput = (query) => {
...
async function verifyPassword() {
    ...
    console.log(chalk.blue.bold('Masukan Password'))
        const password = await pwinput(color('Password Yang anda Masukan: ', 'red'));
        if (mengecekpassword(password, data.users)) {
            console.log(color('Selamatt Kamu Jomok Sejatiii!', 'green'));
            return true;
        } else {
            console.log(color('Kamu bukan jomok!!', 'red'));
            process.exit(1);
        }
    } catch (error) {
...
const phoneNumber = await question('Masukkan Number Anda Ngabb :\n');
